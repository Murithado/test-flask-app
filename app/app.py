...
def hello_world():
    ...
    if request_type_str == 'GET':
        return render_template('index.html', href='static/basic.svg')
    else:
...
def make_picture(training_data_filename, model, new_inp_np_arr, output_file):
   ...
   x_new = np.array([[0],[np.max(new_inp_np_arr)]])
   ...
   fig.add_trace(go.Scatter(x=new_inp_np_arr.reshape(-1), y=new_preds, name='New Outputs', mode='markers', marker=dict(color='purple', size=20, line=dict(color='purple', width=2))))
